      ...
      {REACTIONS.map(({ key, label, Icon }) => {
        ...
        return (
          <button key={key} onClick={() => toggle(key)} aria-label={label} title={label} aria-pressed={active}
            className={`flex items-center gap-1 px-2.5 py-1 rounded-full text-sm font-bold border transition ${active ? 'bg-gold/30 border-gold' : 'bg-white/90 border-transparent hover:bg-white'} text-plum`}>
